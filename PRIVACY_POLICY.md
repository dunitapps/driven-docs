...
- Driven has no developer-operated account system, advertising network, behavioral analytics SDK, or cross-app tracking SDK. The developer operates one single-purpose backend, and it serves road data only.
...
- If **Road Data Lookups** is enabled and paid road-data features apply, location-derived areas or sampled route coordinates are sent to the developer's road-data backend to retrieve OpenStreetMap speed-camera and road speed-limit data. No third-party lookup service receives them.
...
- Dashcam recordings are video only, contain no audio, stay in the App's container on the device, are excluded from device backup, and are never uploaded. The driver alone decides whether to record and whether to export a recording.
- When onboarding starts, the App downloads a small settings file from GitHub. The request contains nothing about the user; GitHub receives the IP address and the App's User-Agent, as with any web request.
...
Having no account system does not mean that nothing leaves the device. Apple, RevenueCat, the developer's road-data backend and its hosting provider, GitHub, artwork hosts, telephone providers, external websites, and email providers process data in the circumstances described below.
...
Road Data Lookups use OpenStreetMap data served by a backend the developer operates. The setting is enabled by default after the App's onboarding disclosure and can be disabled at any time in **Driven > Settings > Privacy > Road Data Lookups**. Disabling it stops Driven's road-data requests and disables speed-camera and road speed-limit features. It does not stop Apple Maps processing that is necessary for map, search, or navigation features.
...
- **Active navigation.** The App samples sections of the planned route and places coordinates in a request at up to six decimal places. The source GPS and route data are less accurate than that formatting may suggest, but the request can reveal the roads being traveled, an approximate origin or destination, and travel direction.

The App contacts a single backend the developer operates, at a subdomain of `dunitapps.com`. No third-party road-data lookup service receives the request. That backend sits behind Cloudflare, which the developer uses as its content-delivery and security provider: Cloudflare terminates the HTTPS connection, so it processes the request content and the source IP address on the developer's behalf before the request reaches the origin server. The backend necessarily processes the request content to return matching OpenStreetMap data and also receives the source IP address, request time, HTTP method, endpoint, protocol and network information, and the App's User-Agent, which gives the App's name, version, and bundle identifier and nothing else. The App does not add a name, email address, Driven account, advertising identifier, RevenueCat identifier, contact, calendar item, or music identifier to a road-data request.

Requests use HTTPS, terminated at Cloudflare's edge, so it protects the request in transit to that edge rather than end-to-end to the origin server. Neither Cloudflare nor the backend requires or accepts a per-user identifier for these requests. The backend keeps no query logs beyond ordinary short-lived operational logging needed to run and secure the service, and answers from a dataset rebuilt periodically from OpenStreetMap rather than forwarding the request to a third party. Cloudflare keeps its own short-term request and security logs under its agreement with the developer.

Responses are cached on the device to reduce repeat network requests. Speed-camera cache entries can be used as fresh for up to 12 hours and, after a network failure, as stale for up to 7 days. Road speed-limit entries can be used as fresh for up to 7 days and as stale for up to 30 days. Each disk cache is pruned at approximately 25 MB and 30 days. Empty-result entries use shorter freshness periods. Cached data may include camera coordinates, OpenStreetMap identifiers, road geometry, speed tags, and timestamps.
...
If Contacts permission is granted, Driven can read contact identifiers, formatted names or organization names, telephone numbers and labels, and available thumbnail images. This is used to display and search callable contacts on the device. The App does not persist the address book to disk and does not upload it to the developer, RevenueCat, or the road-data backend.
...
Apple processes music-library, catalog, search, account, recommendation, and playback information as described in [Apple Music & Privacy](https://www.apple.com/legal/privacy/data/en/apple-music/). Driven does not send Apple Music data to the developer, RevenueCat, or the road-data backend.
...
### 3.8 Dashcam recordings

If Camera permission is granted, Driven can record video of the road ahead while driving. Dashcam recording belongs to the paid Driven Pro feature set, so the camera runs only while paid access applies; during the short window after launch when entitlement status has not yet been confirmed, access is treated as granted so a subscriber is not cut off mid-drive. The recording contains **video only**. Driven does not request microphone access and records no audio. Recording starts only when the driver starts it, or automatically if that setting is enabled, and does not run while the App is in the background.

Recordings are written to the App's private container on the device, in a rolling loop directory and a separate directory for clips the driver chooses to keep. Both are marked as excluded from device backup, so recordings are not copied into iCloud or iTunes/Finder backups. A recording consumes roughly 45 MB per minute at the default quality.

The loop keeps only the most recent footage for the length the driver selects, between 15 and 120 minutes, with 30 minutes as the default. Older segments are deleted automatically as recording continues. Clips the driver saves are kept outside the loop and remain until deleted in the App, and a saved clip is stored with a small record of when it was saved and what moment it covers. Recordings and saved clips can be deleted individually or all at once from the Recordings screen, and removing the App deletes all of them.

**Driven does not upload recordings.** No recording, frame, thumbnail, or derived image is sent to the developer, RevenueCat, the road-data backend, Cloudflare, or any analytics service, and Driven sends nothing to Apple on its own. Footage reaches an Apple service only through an export the user performs, as described below. Recordings are not used to train anything. The developer has no access to them and cannot retrieve, restore, or delete a recording on a user's behalf.

Footage can contain personal data about other people, including vehicles, number plates, faces, pedestrians, building exteriors, and the places and times the vehicle travelled. The driver chooses whether to record at all and what happens to the footage afterwards.

The App offers export of a recording to the Photos library, which requires the separate add-to-Photos permission, or to Files through the system share interface. Export is always a deliberate action by the user. Once a recording leaves the App this way, it is governed by the user's own Photos, iCloud, and Files settings — including any iCloud Photos synchronisation — and by wherever the user then sends it, rather than by this Policy.

A user who publishes or shares footage of identifiable people may become a controller of that personal data under the GDPR, the UK GDPR, or similar law, with their own obligations. The Terms of Service set out this responsibility and the local-law restrictions on dashcam use.

### 3.9 Wallpaper photos
...
### 3.10 Preferences, local caches, and device state
...
### 3.11 Support and legal communications
...
### 3.12 Diagnostics and App Store information
...
Driven uses local system logging for limited operational errors, such as RevenueCat configuration or road-data request failures. The App does not intentionally log live GPS coordinates, contact content, calendar content, music searches, or wallpaper images. Road-cache keys or endpoint/error details can appear in device diagnostic logs.
...
| Send location-derived road-data requests when Road Data Lookups is enabled | User choice/consent where required; performance of the requested road-data feature |
| Record, store, play back, and export dashcam video at the driver's request | Consent through the Camera and add-to-Photos permissions; performance of the requested recording feature |
...
- **Apple:** App distribution, payment, StoreKit, MapKit, Core Location platform services, MusicKit, MediaPlayer, Photos Picker, the Photos library and Files when the user exports a dashcam recording, device backup, system Phone, and optional Apple diagnostics. Apple generally acts under its own terms and privacy notices.
...
- **OpenStreetMap:** source of road, speed-limit, and speed-camera data, obtained through a backend the developer operates rather than a third-party lookup service. Driven reads data; it does not use the App to edit the user's OpenStreetMap account. OpenStreetMap data is licensed under ODbL. See [OpenStreetMap copyright](https://www.openstreetmap.org/copyright).
- **Cloudflare and the developer's hosting provider:** deliver, cache, and protect the road-data backend. They act as the developer's service providers/processors and receive road-data request content and network metadata in order to do so.
- **GitHub:** hosts a settings file the App downloads when onboarding starts. It receives the ordinary request, including the source IP address and the App's User-Agent, under GitHub's own privacy statement.
...
- **Dashcam loop recordings:** only the most recent 15 to 120 minutes, 30 minutes by default; older segments are deleted automatically as recording continues.
- **Saved dashcam clips:** until deleted in the App or the App is removed. Neither loop nor saved recordings are included in device backups.
...
- **Road Data Lookups:** turn the setting off inside Driven to stop location-derived road-data requests. This does not disable Apple Maps processing.
...
- **Dashcam:** recording never starts on its own unless the driver enables automatic start; stop recording at any time, choose the loop length, delete individual or all recordings in the Recordings screen, and revoke Camera or Photos access in iOS Settings. Removing the App deletes every local recording.
...
The controller is in Serbia. Apple, RevenueCat, the developer's own hosting provider, Cloudflare, content delivery hosts, and email infrastructure can process data in Serbia, the European Economic Area, the United States, and other countries where they or their service providers operate. The road-data origin server is hosted in Germany, but Cloudflare answers from whichever of its global edge locations is nearest the user, so a road-data request is normally processed first in or near the user's own country. Those countries can have different privacy laws.

Where the developer appoints a processor and a restricted international transfer applies, the developer relies on an approved legal mechanism where required, such as contractual safeguards, and evaluates supplementary measures as appropriate. Independent providers determine their own transfer mechanisms under their notices. Road Data Lookups are answered by developer-operated infrastructure rather than an independent third-party processor; users can still stop this processing entirely by disabling Road Data Lookups.

## 10. Security

The App uses iOS sandboxing, platform permission controls, HTTPS for road-data requests, platform network-security controls for artwork downloads, local file protection provided by iOS, backup exclusion for dashcam recordings, data minimization, and limited dependencies. No system is completely secure. The developer cannot guarantee the security or uninterrupted availability of a device, backup, carrier, hosting provider, Apple service, RevenueCat, external website, or email transmission.
...
Driven is a driving-oriented product and is not directed to children under 16. A minor may use the App only with the involvement and authorization of a parent or legal guardian and only where lawful. The developer does not knowingly seek a child's contact, calendar, location, music, photo, or camera data for advertising or profiling. A guardian who believes a child supplied data to the developer may contact [contact@dunitapps.com](mailto:contact@dunitapps.com).
